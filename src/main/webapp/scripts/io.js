...
function printItems(respObjList) {
	...
	for(let item of respObjList) {
		...
		htmlStr+="<span class='poista' onclick=varmistaPoisto("+item.asiakas_id+",'"+encodeURI(item.etunimi)+"','"+encodeURI(item.sukunimi)+"')>Poista</span></td>"; //encodeURI() muutetaan erikoismerkit, välilyönnit jne. UTF-8 merkeiksi.
    	htmlStr+="</tr>";    
	}
    ...
    .then(response => response.json())//Muutetaan vastausteksti JSON-objektiksi
   	.then(responseObj => {	
   		...
   		if(responseObj.response==0){
			...
			document.lomake.reset(); //Tyhjennetään asiakkan muutos lomake		        	
		}
   	})
   	.catch(errorText => console.error("Fetch failed: " + errorText));
}
